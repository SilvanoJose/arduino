...
void reconnect() {
  // Imprime o status atual da conexão MQTT
  Serial.print("Status da conexão MQTT: ");
  Serial.println(client.state());

  // Verifica se não está conectado e tenta se reconectar
  if (!client.connected()) {
...
File file3 = SPIFFS.open("/led_state3.txt", "r");
  ...
  //client.setTimeout(5000); // Defina o tempo limite para 5 segundos (ou o valor desejado)
  //client.setCleanSession(true); // Habilita a limpeza da sessão MQTT
...
void loop() {
  ...
  if (currentMillis - previousMillis >= interval) {
    ...
    readTemperature();
    checkSchedule();
    //Serial.println("Passou pelo checkSchedule");
  }
  
  //Serial.print("Status da conexão MQTT dentro do loop, antes de verificar se está conectado: ");
  //Serial.println(client.state());
  if (!client.connected()) {
    Serial.print("Status da conexão MQTT dentro do loop, quando verificou que não está conectado e vai chamar reconnect: ");
    Serial.println(client.state());
    reconnect();
    Serial.print("Status da conexão MQTT dentro do loop, depois de chamar reconnect: ");
    Serial.println(client.state());
  }
  client.loop();
